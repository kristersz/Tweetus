...
namespace AspNet.Identity3.MongoDB
{
        ...
        where TContext : MongoIdentityContext<TUser, TRole>
    {
        ...
        public virtual Task<TUser> FindByIdAsync(string userId, CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            var filter = Builders<TUser>.Filter.Eq("Id", userId);
        ...
        public virtual Task<TUser> FindByNameAsync(string normalizedUserName, CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            var filter = Builders<TUser>.Filter.Eq("NormalizedUserName", normalizedUserName);
            return Context.Users.Find(filter).FirstOrDefaultAsync(cancellationToken);
            ...
            CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            var builder = Builders<TUser>.Filter;
            var filter = builder.Eq("Logins.LoginProvider", loginProvider) & builder.Eq("Logins.ProviderKey", providerKey);
            return await Context.Users.Find(filter).FirstOrDefaultAsync(cancellationToken);
